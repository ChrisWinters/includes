...
trait Trait_Option_Manager {
	...
	public function get_option() {
		 ...
		 */
		$get_option = get_option( INCLUDES_PLUGIN_NAME );
	...
	public function del_option() {
		...
		if ( true === isset( $get_option ) ) {
			delete_option( INCLUDES_PLUGIN_NAME );
		}
	}//end del_option()
	...
	public function del_setting( $setting_name ) {
		...
		if ( true !== empty( $get_option )  && isset( $get_option ) ) {
			$this->del_option();
		}
	}//end del_setting()
	...
	public function all_options() {
		...
		if ( defined( 'WP_DEBUG' ) && true === WP_DEBUG ) {
			...
			foreach ( $loaded_options as $name => $value ) {
				if ( false !== stristr( $name, INCLUDES_SETTING_PREFIX ) ) {
					$options[ $name ] = $value;
				}
